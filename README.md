...
| Finger_Search.py | Implementations of the finger search algorithms on balanced binary trees with predesessor and successor, as discussed in the paper, as well as the new additions. |
...
| Level\_Linked\_ab\_Tree.py | Implementation of the Level Linked trees by  Huddleston and Mehlhorn, which allows for finger searching. This is created to have a basline for comparison of the finger search algorithms. Note that this structure uses a different finger search algorith then the ones on binary trees, and that the structure does not have *pure nodes*. The paper is [available here](https://doi.org/10.1007/BF00288968). The implementation only allows for insertions and not deletions. |
