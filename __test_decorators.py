...
def test_group(f):
    def g(*args, **kwargs):
        global CURRENT_INDENT, GROUP_NESTING, PASS_GROUP_COUNT, FAIL_GROUP_COUNT
        PASS_GROUP_COUNT = FAIL_GROUP_COUNT = 0
        ...
        try:
            ...
            MISSING_IMPL = True
            print(f"{INDENT()}{YELLOW}Missing Implementation{STOP}")
        CURRENT_INDENT -= 1
        GROUP_NESTING -= 1
        total_group_count = PASS_GROUP_COUNT + FAIL_GROUP_COUNT
        print(f"{INDENT()}{PASS if FAIL_GROUP_COUNT == 0 and not MISSING_IMPL else FAIL} {f.__name__} ({PASS_GROUP_COUNT}/{total_group_count})")
        if GROUP_NESTING == 0: print()
...
# ---------------------------------------------
#  Local tests used for testing the decorators
# ---------------------------------------------
...
@test_group
def sample_group_with_group():
    sample_group()

@test_group
def sample_group_not_implemented():
    raise NotImplementedError

@test
def sample_test_not_implemented():
    raise NotImplementedError

@test_group
def sample_group_with_not_implemented():
    sample_test_positive()
    sample_test_not_implemented()
...
def sample_file():
    ...
    sample_group_with_group()
    sample_group_not_implemented()
    sample_group_with_not_implemented()
